...
// ------------------------
	...
	server->begin();
	#define SDCARD_SS_PIN chipSelectPin
...
// ------------------------
	Serial.print("Rejecting request: "); Serial.println(rejectMessage);
...
// ------------------------
	...
	DBG_PRINT(" u: "); Serial.println(uri);
...
// ------------------------
	Serial.println("Processing OPTION");
...
// ------------------------
	Serial.println("Processing LOCK");
...
// ------------------------
	Serial.println("Processing UNLOCK");
	sendHeader("Allow", "PROPPATCH,PROPFIND,OPTIONS,DELETE,UNLOCK,COPY,LOCK,MOVE,HEAD,POST,PUT,GET");
	sendHeader("Lock-Token", "urn:uuid:26e57cb3-834d-191a-00de-000042bdecf9");
...
// ------------------------
	Serial.println("PROPPATCH forwarding to PROPFIND");
	handleProp(resource);
}
...
// ------------------------
	Serial.println("Processing PROPFIND");
	// check depth header
	...
	DBG_PRINT("Depth: "); Serial.println(depth);
...
// String fullResPath = "http://" + hostHeader + uri;
	...
	tm tmStr;
	tmStr.tm_hour = uint16_t(FS_HOUR(dir.modifyTime[0]));
	tmStr.tm_min = FS_MINUTE(dir.modifyTime[0]);
	tmStr.tm_sec = FS_SECOND(dir.modifyTime[0]);
	tmStr.tm_year = FS_YEAR(dir.modifyDate[0]) - 1900;
	tmStr.tm_mon =  FS_MONTH(dir.modifyDate[0]) - 1;
	tmStr.tm_mday = FS_DAY(dir.modifyDate[0]);
...
// ------------------------
	...
	rFile.close();
	DBG_PRINT("File "); DBG_PRINT(fileSize); DBG_PRINT(" bytes sent in: "); DBG_PRINT((millis() - tStart)/1000); Serial.println(" sec");
}
...
// ------------------------
	...
	DBG_PRINT(uri); Serial.println(" - ready for data");
	...
	if(contentLen != 0)	{
		...
		DBG_PRINT("File "); DBG_PRINT(contentLen - numRemaining); DBG_PRINT(" bytes stored in: "); DBG_PRINT((millis() - tStart)/1000); Serial.println(" sec");
	}
...
// ------------------------
	Serial.println("Processing MKCOL");
	...
	DBG_PRINT(uri);	Serial.println(" directory created");
...
// ------------------------
	Serial.println("Processing DELETE");
	...
	if(!retVal)	{
		...
		Serial.println("Unable to delete file/directory");
		return;
	}
...
// ------------------------
bool ESPWebDAV::initSD(int chipSelectPin) {
	// initialize the SD card
	return sd.begin(chipSelectPin);
}
...
// ------------------------
	...
	// does URI refer to an existing resource
	if(resource == RESOURCE_NONE) return handleNotFound();
	...
	// open this resource
	File32 baseFile;
	     if (!baseFile.open(uri.c_str())){
    Serial.println("dir.open failed");
  }else{
	//baseFile.open(uri.c_str());
	sendPropResponse(false, &baseFile);
...
// String fullResPath = "http://" + hostHeader + uri;
	...
	//if(recursing)
		//if(fullResPath.endsWith(uri.c_str()))
			//fullResPath += String(buf);
		//else
			//fullResPath += "/" + String(buf);
fullResPath=String(buf);
...
// ------------------------
	...
	// does URI refer to an existing file resource
	DBG_PRINT(" RESOURCE_FILE "); DBG_PRINTLN(RESOURCE_FILE);
	DBG_PRINT(" resource"); DBG_PRINTLN(resource);
	if(resource != RESOURCE_FILE)
		return handleNotFound();
